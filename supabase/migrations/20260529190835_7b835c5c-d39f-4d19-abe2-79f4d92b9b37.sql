...
CREATE OR REPLACE FUNCTION public.get_nfc_badge(_code text)
 RETURNS TABLE(employee_id uuid, employee_code text, display_name text, preferred_name text, job_title text, credential text, pronouns text, photo_url text, email text, phone text, extension text, meeting_link text, department_name text, states text[], state text, bio text, about_me text, expertise text[], skills text[], languages text[], help_with text[], linkedin_url text, leadership_level text, emergency_contact jsonb, nfc_settings jsonb, badge_style text, role_key text)
 LANGUAGE sql
