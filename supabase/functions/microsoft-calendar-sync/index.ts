// Per-user Microsoft Calendar sync. Refreshes the user's vaulted token via
// _shared/microsoftTokenVault, then pages a small window of upcoming events.
...
Deno.serve(async (req) => {
  ...
  // Decrypt the just-rotated access token from the vault row.
  ...
  const res = await fetch("https://graph.microsoft.com/v1.0/me/calendar/events?$top=25&$orderby=start/dateTime", {
  ...
  for (const ev of events) {
    const up = await upsertNormalizedRecord({ supabase }, "ms365", {
      ...
      recordStatus: ev.showAs ?? null,
      displayTitle: ev.subject ?? "(no subject)",
      occurredAt: ev.start?.dateTime ? new Date(ev.start.dateTime).toISOString() : null,
      personEmail: ev.organizer?.emailAddress?.address ?? null,
      personName: ev.organizer?.emailAddress?.name ?? null,
      sourceLabel: "Microsoft Outlook Calendar",
      externalUrl: ev.webLink ?? null,
      metadata: {
        user_id: user.id,
        attendees_count: Array.isArray(ev.attendees) ? ev.attendees.length : 0,
        end: ev.end?.dateTime,
        is_online_meeting: ev.isOnlineMeeting,
      },
    });
    if (up.ok) created += 1;
  }
  return json({ ok: true, received: events.length, created });
});