...
describe("canonical V3 ownership adapter", () => {
  const result = {
    ...
    conflicts: [],
    gaps: [],
    clientsWithoutAnchors: [],
  } as unknown as OwnershipResult;

  const index = buildCanonicalOwnershipIndex(result);

  it("resolves by CentralReach client id first", () => {
    const r = index.resolve({ clientCrId: "CR-100", clientName: "Wrong Name", date: "2026-03-11" });
    expect(r.bcba).toBe("Dr. Reed");
    expect(r.matchedBy).toBe("client_cr_id");
  });

  it("falls back to the client name when no id is present", () => {
    const r = index.resolve({ clientName: "alex  doe", date: "2026-04-02" });
    expect(r.bcba).toBe("Dr. Kim");
    expect(r.matchedBy).toBe("client_name");
  });

  it("respects the month a session happened in", () => {
    expect(index.resolve({ clientCrId: "CR-100", date: "2026-04-20" }).bcba).toBe("Dr. Kim");
  });

  it("returns null rather than guessing for an unknown client", () => {
    expect(index.resolve({ clientName: "Nobody" }).bcba).toBeNull();
  });
});
