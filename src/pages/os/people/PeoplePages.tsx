...
const SAMPLE_EMPLOYEES = [
  { name: "Lauren Brewer", role: "Executive / Owner", dept: "Executive", state: "GA", email: "lauren@blossomaba.com", phone: "—", status: "Active", devices: "MacBook Pro", badge: "Active" },
  { name: "Briana Diaz", role: "Operations Leadership", dept: "Operations", state: "Multi", email: "briana@blossomaba.com", phone: "—", status: "Active", devices: "MacBook Air", badge: "Active" },
  { name: "Maria Lopez", role: "Intake Coordinator", dept: "Intake", state: "GA", email: "intake@blossomaba.com", phone: "—", status: "Active", devices: "iPad", badge: "Pending" },
  { name: "Devon Ross", role: "Scheduling Lead", dept: "Scheduling", state: "NC", email: "scheduling@blossomaba.com", phone: "—", status: "Active", devices: "MacBook Air", badge: "Active" },
  { name: "Ashley Tran", role: "BCBA", dept: "Clinical", state: "VA", email: "atran@blossomaba.com", phone: "—", status: "Active", devices: "iPad", badge: "Active" },
  { name: "Jordan Park", role: "RBT", dept: "Clinical", state: "TN", email: "jpark@blossomaba.com", phone: "—", status: "Active", devices: "—", badge: "Replacement needed" },
];

export function EmployeeDirectoryPage() {
  const [q, setQ] = useState("");
  const filtered = SAMPLE_EMPLOYEES.filter((e) =>
    [e.name, e.role, e.dept, e.state].join(" ").toLowerCase().includes(q.toLowerCase()),
  );
  return (
    <Shell>
      <PageHeader
        ...
        subtitle="Search Blossom team members, roles, departments, states, contact information, and assigned resources."
        icon={Users}
      />
      <ActionRow actions={["Search employee", "Filter by department", "Filter by role", "Open employee profile", "View assigned assets"]} />
      <div className="relative max-w-md">
        <Search className="h-4 w-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search employees..." className="pl-9 h-10" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                {["Name", "Role", "Department", "State", "Email", "Phone", "Status", "Devices", "NFC Badge"].map((h) => (
            ...
            <tbody>
              {filtered.map((e) => (
                <tr key={e.email} className="border-t border-border/60 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><Badge variant="outline">{e.status}</Badge></td>
                  <td className="px-4 py-3 text-muted-foreground">{e.devices}</td>
                  <td className="px-4 py-3">
                    <Badge variant="outline" className={cn(
                      e.badge === "Active" && "bg-emerald-50 text-emerald-700 border-emerald-200",
                      e.badge === "Pending" && "bg-amber-50 text-amber-800 border-amber-200",
                      e.badge === "Replacement needed" && "bg-red-50 text-red-700 border-red-200",
                    )}>{e.badge}</Badge>
                  </td>
                </tr>
              ))}
