...
/**
 * OS Onboarding — the canonical onboarding experience surfaced inside Blossom OS.
 * Mirrors the /onboarding/phase/welcome Journey (Welcome → Week 1 → Week 2 →
 * Week 3 → Weeks 4–5 → Graduation) using the same phase data + components, but
 * presented inside the OS shell so it lives alongside the HR Training Academy.
 */
export default function OSOnboarding() {
  ...
  const completedPhases = status.phaseProgress.filter(
    (p) => p.complete && p.phase.id !== "graduation",
  ).length;
  const totalPhases = status.phaseProgress.filter(
    (p) => p.phase.id !== "graduation",
  ).length;

  return (
    <OSShell>
      <div className="px-6 md:px-10 py-10 max-w-6xl mx-auto space-y-6 pb-16">
        {/* Hero */}
          ...
          <div className="relative space-y-4">
            ...
            <div className="grid max-w-2xl gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-border/60 bg-background/70 p-3 backdrop-blur">
                ...
                <p className="mt-1 text-xl font-semibold tabular-nums text-foreground">
                  {completedPhases}
                  <span className="text-sm font-normal text-muted-foreground">/{totalPhases}</span>
                </p>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1">
              {!status.isComplete && status.nextPhase && (
                <Button asChild size="lg" className="w-full gap-2 rounded-2xl shadow-lg shadow-primary/20 sm:w-auto">
                      ...
                      {status.moduleDoneCount === 0 ? (
                        ...
                        <>
                          ...
                          <span className="hidden sm:inline">
                            Continue: {status.nextPhase.weekLabel} — {status.nextPhase.title}
                          </span>
                        </>
                      )}
                    </span>
            ...
            {status.nextPhase && !status.isComplete && (
              <section className="rounded-2xl border border-primary/30 bg-[linear-gradient(135deg,hsl(var(--primary)/0.08),transparent)] p-4 shadow-sm">
                ...
                <p className="mt-1 text-sm font-semibold text-foreground">
                  {status.nextPhase.weekLabel} — {status.nextPhase.title}
                </p>
                ...
                <Button asChild size="sm" className="mt-3 w-full gap-1 rounded-xl">
                  <Link to={status.nextPhase.path}>
                    Open phase <ArrowRight className="h-3.5 w-3.5" />
                  </Link>
                </Button>
              </section>
            )}
