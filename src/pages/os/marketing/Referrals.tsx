    ...
    >
      ...
      <MktgCard>
        <TableFilterBar
          ...
          filters={[
            { key: "state", label: "State", value: stateFilter, onChange: setStateFilter, countSource: activeTab === "companies" ? (companies as unknown[]) : (contacts as unknown[]), countValue: (c) => (activeTab === "companies" ? (c as ReferralCompany).state ?? "" : (c as ReferralContact).state ?? ""), options: [{ value: "all", label: "All states" }, ...states.map((s) => ({ value: s, label: s }))] },
            { key: "stage", label: "Stage", value: stageFilter, onChange: setStageFilter, countSource: contacts as unknown[], countValue: (c) => (c as ReferralContact).relationship_stage ?? "", options: [{ value: "all", label: "All stages" }, ...["New Contact", "First Outreach", "Connected", "Active Referral Source", "Strong Partner", "Needs Follow-Up", "Dormant"].map((v) => ({ value: v, label: v }))], width: 180 },
            { key: "owner", label: "Owner", value: ownerFilter, onChange: setOwnerFilter, countSource: activeTab === "companies" ? (companies as unknown[]) : (contacts as unknown[]), countValue: (c) => activeTab === "companies" ? (ownersToList((c as ReferralCompany).relationship_owner).length ? ownersToList((c as ReferralCompany).relationship_owner) : ["__unassigned__"]) : (ownersToList((c as ReferralContact).contact_owner).length ? ownersToList((c as ReferralContact).contact_owner) : ["__unassigned__"]), options: [{ value: "all", label: "All owners" }, { value: "__unassigned__", label: "Unassigned" }, ...Array.from(new Set([...contactOwners, ...companyOwners])).map((o) => ({ value: o, label: o }))], width: 170 },
          ]}
