  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {viewMode === "coverage" && <div className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm"><table className="w-full min-w-[980px] text-sm"><thead><tr className="border-b border-border/50 bg-muted/20 text-left text-[11px] uppercase tracking-wide text-muted-foreground">{["State", "Region", "Clinic", "Required Client Hours", "Available RBT Hours", "Assigned Hours", "Gap", "Coverage", "Unstaffed Clients", "Available RBTs"].map((h) => <th key={h} className="px-4 py-3 font-medium">{h}</th>)}</tr></thead><tbody>{coverageRows.map((row) => <tr key={`${row.state}-${row.region}-${row.clinic}`} className="border-b border-border/40 hover:bg-muted/25"><td className="px-4 py-3 font-medium text-foreground">{row.state}</td><td className="px-4 py-3 text-muted-foreground">{row.region}</td><td className="px-4 py-3 text-muted-foreground">{row.clinic}</td><td className="px-4 py-3 text-muted-foreground">{row.required}h</td><td className="px-4 py-3 text-muted-foreground">{row.available}h</td><td className="px-4 py-3 text-muted-foreground">{row.assigned}h</td><td className={cn("px-4 py-3 font-semibold", row.gap < 0 ? "text-destructive" : "text-success")}>{row.gap}h</td><td className="px-4 py-3"><div className="flex items-center gap-2"><Progress value={row.coverage} className="h-2 w-24" /><span className="text-xs text-muted-foreground">{row.coverage}%</span></div></td><td className="px-4 py-3 text-muted-foreground">{row.unstaffed}</td><td className="px-4 py-3 text-muted-foreground">{row.rbts}</td></tr>)}</tbody></table></div>}
      </div>
