  ...
  return (
    <section className="mt-6 overflow-hidden rounded-lg border bg-card shadow-sm">
      ...
      <div className="grid gap-0 xl:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <aside className="border-t p-4 xl:border-l xl:border-t-0"><div className="rounded-lg border bg-muted/20 p-3"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{selected.client}</p><p className="text-xs text-muted-foreground">{selected.region} · {selected.location} · {selected.requiredHours}h needed</p></div><StatusPill tone={urgencyTone(selected.priority)}>{selected.priority}</StatusPill></div>{activeMatch && selectedRoute && <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-md border bg-background p-2"><Navigation className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{selectedRoute.miles} mi</p><p className="text-muted-foreground">drive</p></div><div className="rounded-md border bg-background p-2"><Clock className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{selectedRoute.minutes}m</p><p className="text-muted-foreground">ETA</p></div><div className="rounded-md border bg-background p-2"><ShieldCheck className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{selectedRoute.withinRadius ? "Inside" : "Outside"}</p><p className="text-muted-foreground">radius</p></div></div>}</div><div className="mt-4 space-y-3"><h3 className="text-sm font-semibold">Fastest viable RBTs</h3>{mapMatches.map(({ match, route }) => <button key={match.rbt.id} onClick={() => onSelectRbt(match.rbt.id)} className={cn("w-full rounded-lg border p-3 text-left transition hover:bg-muted/40", activeMatch?.rbt.id === match.rbt.id && "border-primary bg-primary/5")}><div className="flex items-center justify-between"><div><p className="font-medium">{match.rbt.name}</p><p className="text-xs text-muted-foreground">{route.minutes} min · {route.miles} mi · {availableHours(match.rbt)}h open</p></div><span className="text-lg font-semibold">{match.score}</span></div><div className="mt-2 flex flex-wrap gap-1.5"><StatusPill tone={route.withinRadius ? "success" : "warning"}>{route.withinRadius ? "Within radius" : "Travel exception"}</StatusPill><StatusPill tone={match.ready ? "success" : "destructive"}>{match.rbt.compliance}</StatusPill></div></button>)}</div><div className="mt-5 space-y-3"><h3 className="text-sm font-semibold">Accessible top match pairs</h3><ol className="space-y-3" aria-label="Top RBT client pairs with route and score details">{topPairs.map(({ record, match, route }, index) => <li key={`${record.id}-${match.rbt.id}`}><button onClick={() => { onSelectRecord(record, match); onOpenDetails(); }} className={cn("w-full rounded-lg border p-3 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring hover:bg-muted/40", selected.id === record.id && activeMatch?.rbt.id === match.rbt.id && "border-primary bg-primary/5")} aria-label={`${index + 1}. ${match.rbt.name} matched with ${record.client}, ${route.minutes} minutes, ${route.miles} miles, score ${match.score}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{match.rbt.name} → {record.client}</p><p className="text-xs text-muted-foreground">{record.region} · {record.location} · {availableHours(match.rbt)}h open</p></div><span className="text-lg font-semibold">{match.score}</span></div><div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-md border bg-background p-2"><p className="font-semibold">{route.minutes}m</p><p className="text-muted-foreground">drive</p></div><div className="rounded-md border bg-background p-2"><p className="font-semibold">{route.miles} mi</p><p className="text-muted-foreground">distance</p></div><div className="rounded-md border bg-background p-2"><p className="font-semibold">{route.withinRadius ? "Inside" : "Outside"}</p><p className="text-muted-foreground">radius</p></div></div><div className="mt-3 grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">{breakdownRows(match.breakdown).map(([label, value]) => <div key={label} className="flex items-center justify-between rounded-md border bg-background px-2 py-1.5"><span>{label}</span><span className="font-medium text-foreground">{value}</span></div>)}</div></button></li>)}</ol></div>{activeMatch && <Button className="mt-4 w-full" onClick={() => onAssign(selected, activeMatch.rbt)}><UserCheck className="mr-2 h-4 w-4" />Assign {activeMatch.rbt.name}</Button>}</aside>
      </div>
    </section>
  );
}
...
export default function StaffingDashboard() {
  ...
  const activeRoute = activeMatch ? routeStats(selected, activeMatch.rbt) : null;
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <Sheet open={detailOpen} onOpenChange={setDetailOpen}><SheetContent className="w-[720px] max-w-[92vw] overflow-y-auto sm:max-w-[720px]"><SheetHeader><SheetTitle>{selected.client}</SheetTitle><SheetDescription>{selected.state} / {selected.clinic} · {selected.status} · {selected.requiredHours} weekly hours</SheetDescription></SheetHeader><div className="mt-4 flex flex-wrap gap-2"><Button size="sm" onClick={() => activeMatch && assignRbt(selected, activeMatch.rbt)}><UserCheck className="mr-2 h-4 w-4" />Assign Top Match</Button><Button variant="outline" size="sm" onClick={() => updateRecord(selected.id, { status: "Matching in Progress" }, "Marked matching in progress")}><Sparkles className="mr-2 h-4 w-4" />Mark Matching</Button><Button variant="outline" size="sm" onClick={() => updateRecord(selected.id, { priority: "Critical", nextAction: "Escalated to leadership" }, "Case escalated")}><AlertTriangle className="mr-2 h-4 w-4" />Escalate</Button></div>{activeMatch && activeRoute && <div className="mt-4 rounded-lg border bg-muted/20 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">{activeMatch.rbt.name} match details</p><p className="text-xs text-muted-foreground">{activeMatch.rbt.location} to {selected.location.toLowerCase()} service · {activeMatch.overlap} schedule overlaps · {availableHours(activeMatch.rbt)}h open</p></div><div className="text-right"><p className="text-2xl font-semibold">{activeMatch.score}</p><p className="text-[11px] text-muted-foreground">match score</p></div></div><div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-md border bg-background p-2"><Navigation className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{activeRoute.miles} mi</p><p className="text-muted-foreground">distance</p></div><div className="rounded-md border bg-background p-2"><Clock className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{activeRoute.minutes}m</p><p className="text-muted-foreground">drive</p></div><div className="rounded-md border bg-background p-2"><Route className="mx-auto mb-1 h-3.5 w-3.5 text-primary" /><p className="font-semibold">{activeRoute.withinRadius ? "Inside" : "Outside"}</p><p className="text-muted-foreground">radius</p></div></div><div className="mt-3 grid grid-cols-4 gap-2 text-[11px] text-muted-foreground">{[["Region", activeMatch.breakdown.region, auditWeights.region], ["Availability", activeMatch.breakdown.availability, auditWeights.availability], ["Compliance", activeMatch.breakdown.compliance, auditWeights.compliance], ["Capacity", activeMatch.breakdown.capacity, auditWeights.capacity]].map(([label, value, max]) => <div key={String(label)} className="rounded-md border bg-background p-2"><div>{label}</div><div className="font-medium text-foreground">{value}/{max}</div></div>)}</div><div className="mt-3 flex flex-wrap gap-2">{activeMatch.reasons.map((reason) => <StatusPill key={reason}>{reason}</StatusPill>)}</div></div>}<Separator className="my-4" /><Tabs defaultValue="overview"><TabsList className="grid w-full grid-cols-4"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="matching">Matching</TabsTrigger><TabsTrigger value="availability">Availability</TabsTrigger><TabsTrigger value="profile">RBT Profile</TabsTrigger></TabsList><TabsList className="mt-2 grid w-full grid-cols-4"><TabsTrigger value="communications">Communications</TabsTrigger><TabsTrigger value="tasks">Tasks</TabsTrigger><TabsTrigger value="timeline">Timeline</TabsTrigger><TabsTrigger value="audit">Audit</TabsTrigger></TabsList>
