...
function ScheduleSidePanel({ record, tab, setTab, onClose, onClient, onBuild, onStart, onCr, onEmail, onDoc, onTask, onPatch }: { record: ScheduleRecord; tab: PanelTab; setTab: (tab: PanelTab) => void; onClose: () => void; onClient: () => void; onBuild: (record: ScheduleRecord) => void; onStart: (record: ScheduleRecord) => void; onCr: (record: ScheduleRecord, status: CRStatus) => void; onEmail: (record: ScheduleRecord) => void; onDoc: (record: ScheduleRecord) => void; onTask: (record: ScheduleRecord, title: string) => void; onPatch: (id: string, patch: Partial<ScheduleRecord>, event: string) => void }) {
  return <aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl"><div className="border-b border-border bg-card p-3 sm:p-5"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-lg font-semibold text-foreground sm:text-xl">{record.clientName}</h2><StatusBadge status={record.status} variant={statusVariant(record.status)} /></div><p className="mt-1 text-xs text-muted-foreground sm:text-sm">{record.state} / {record.clinic} · {record.bcba} · {record.rbt} · {record.scheduler}</p><div className="mt-3 flex flex-wrap gap-2"><StatusBadge status={`Start ${record.startDate ?? "missing"}`} variant={record.startDate ? "info" : "destructive"} /><StatusBadge status={`${riskFor(record)} risk`} variant={riskFor(record) === "Critical" ? "destructive" : riskFor(record) === "High" ? "warning" : "info"} /><StatusBadge status={record.crStatus} variant={crVariant(record.crStatus)} /></div></div><button onClick={onClose} className="shrink-0 rounded-md p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><X className="h-4 w-4" /></button></div><div className="-mx-3 mt-4 flex gap-2 overflow-x-auto px-3 pb-1 sm:mx-0 sm:flex-wrap sm:px-0 sm:pb-0"><Button size="sm" variant="outline" className="shrink-0" onClick={onClient}><UserRoundCheck className="h-3.5 w-3.5" />Open Client</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onBuild(record)}>Build Schedule</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onStart(record)}>Set Start Date</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onCr(record, "Entered")}>Mark CR Entered</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onEmail(record)}>Send Pairing Email</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onDoc(record)}>Generate Case Coordination Doc</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onTask(record, "Create scheduling task")}>Create Task</Button><Button size="sm" variant="outline" className="shrink-0" onClick={() => onTask(record, "Add scheduling note")}>Add Note</Button></div></div><div className="border-b border-border bg-card px-3 py-2 sm:px-5"><div className="flex gap-1 overflow-x-auto">{panelTabs.map((item) => <button key={item} onClick={() => setTab(item)} className={cn("whitespace-nowrap rounded-md px-3 py-2 text-xs font-medium transition-colors", tab === item ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground")}>{item}</button>)}</div></div><div className="flex-1 overflow-y-auto p-3 sm:p-5"><PanelContent record={record} tab={tab} onPatch={onPatch} onCr={onCr} onEmail={onEmail} /></div></aside>;
}
