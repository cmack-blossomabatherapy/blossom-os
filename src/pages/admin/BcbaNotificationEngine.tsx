...
import { Bell, Pause, Play, AlertTriangle, History, ShieldCheck } from "lucide-react";

type Rule = {
  id: string; event_key: string; domain: string; category: string;
  title_template: string; body_template: string; channels: string[];
  required: boolean; sensitive: boolean; respect_quiet_hours: boolean;
  dedupe_window_minutes: number; active: boolean;
  paused_at: string | null; paused_reason: string | null;
  action_label: string | null; deep_link_template: string | null;
};

export default function BcbaNotificationEngine() {
  ...
  const load = async () => {
    const [r, d, a] = await Promise.all([
      supabase.from("bcba_notification_rules").select("*").order("domain").order("event_key"),
      supabase.from("bcba_notification_deliveries").select("*").in("status", ["failed", "pending"]).order("created_at", { ascending: false }).limit(100),
      supabase.from("bcba_notification_audit").select("*").order("created_at", { ascending: false }).limit(100),
    ]);
  ...
  const togglePause = async (r: Rule) => {
    ...
    const { error } = await supabase.from("bcba_notification_rules").update({
      paused_at: paused ? null : new Date().toISOString(),
      paused_by: paused ? null : user?.id ?? null,
    }).eq("id", r.id);
    if (error) { toast.error(error.message); return; }
    await supabase.from("bcba_notification_audit").insert({
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 p-4 pb-16 sm:p-6">
      <header>
        <div className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          <Bell className="h-3.5 w-3.5" /> BCBA Notification Engine
        </div>
        <h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Notifications &amp; automations</h1>
        <p className="text-sm text-muted-foreground">Configure rules, pause automations, protect sensitive content, and monitor failed deliveries.</p>
      </header>
                  ...
                  <div key={r.id} className="flex flex-col gap-2 rounded-xl border border-border/60 p-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {r.sensitive && <Badge className="text-[10px]" variant="secondary"><ShieldCheck className="mr-1 h-3 w-3" />Sensitive</Badge>}
                    ...
                    <Button size="sm" variant="outline" disabled={r.required} onClick={() => togglePause(r)}>
