...
const SECTIONS: StaffSection[] = [
  { id: "start_here",   tag: "staff_start_here",   title: "Start Here",                              subtitle: "Staffing inventory, department SOP book, and Staffing Coordinator role SOP." },
  { id: "sops",         tag: "staff_sops",         title: "Core Staffing SOPs",                      subtitle: "Case match, hours serviced, open case follow-up, RBT availability, escalation, and full SOP book." },
  { id: "open_cases",   tag: "staff_open_cases",   title: "Open Cases & Case Matching",              subtitle: "Pairing process and family staffing preferences." },
  { id: "coverage",     tag: "staff_coverage",     title: "RBT Availability & Coverage",             subtitle: "Coverage gaps, coverage risks, and Blossom's staffing structure." },
  { id: "kpis",         tag: "staff_kpis",         title: "Hours Serviced & Staffing KPIs",          subtitle: "Staffing KPI academy guide and Staffing playbook." },
  { id: "escalations",  tag: "staff_escalations",  title: "Escalations & Recruiting Handoff",        subtitle: "Escalations academy guide and Staffing → Recruiting handoff references." },
  { id: "training",     tag: "staff_training",     title: "Training, Shadowing & Role Packet",       subtitle: "Onboarding journey, current packet, job packet, acknowledgement, and field manual." },
  { id: "reports",      tag: "staff_reports",      title: "Reports, Exports & Examples",             subtitle: "Owner/review worklist snapshot — leadership reference only.", leadershipOnly: true },
  { id: "videos",       tag: "staff_videos",       title: "Videos & Media",                          subtitle: "No Staffing-specific videos have been uploaded yet." },
  { id: "admin_qa",     tag: "staff_admin_qa",     title: "Needs Review — Staffing Adjacent / Future", subtitle: "Future-facing planning materials, owner confirmation, disk audit, and admin prompts.", adminOnly: true },
];

function isLeadershipRole(role: string): boolean {
  return [
    "super_admin","executive_leadership","operations_leadership","training_management",
    "doo","director_of_operations","state_director","assistant_state_director","staffing_lead",
  ].includes(role);
}
...
export default function ResourceLibraryStaffing() {
  ...
  const staffResources = useMemo(
    () => resources.filter((r) => {
      ...
      return depts.includes("staffing") || (r.tags ?? []).some((t) => t?.startsWith("staff_"));
    }),
  ...
  const filtered = useMemo(() => {
    ...
    return staffResources.filter((r) => {
      if (q) {
  ...
  }, [staffResources, query, fileType, audience, sectionFilter]);

  const grouped = useMemo(() => {
    const map: Record<string, typeof filtered> = {};
    for (const s of SECTIONS) map[s.id] = [];
    for (const r of filtered) {
      const tag = (r.tags ?? []).find((t) => t?.startsWith("staff_") && SECTIONS.some((s) => s.tag === t));
  ...
  const missing = staffResources.filter((r) => !r.storagePath && !r.url).length;
    ...
    <OSShell>
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        <header className="rounded-3xl border border-border/60 bg-card/70 p-6 shadow-sm">
          ...
          <h1 className="mt-1 text-2xl font-semibold tracking-tight">Staffing Resources</h1>
          <p className="mt-1 max-w-3xl text-[13px] text-muted-foreground">
            Every current-state Staffing SOP, playbook, academy guide, and role packet — organized so the
            Staffing team and trainees can find, open, or download exactly what they need.
          </p>
        </header>
        <LibraryTabs />
        ...
        <div className="grid gap-3 rounded-2xl border border-border/60 bg-card/70 p-3 sm:grid-cols-[1fr_auto_auto_auto]">
          <div className="relative">
            ...
            <Input
              placeholder="Search — open case, RBT availability, coverage, pairing, recruiting escalation, KPI, packet…"
              value={query}
          ...
          <Select value={sectionFilter} onValueChange={setSectionFilter}>
            <SelectTrigger className="h-9 w-[220px] rounded-xl text-[12.5px]"><SelectValue placeholder="Section" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="flex flex-wrap gap-2 text-[11.5px] text-muted-foreground">
          ...
          <span className="rounded-full bg-muted px-2 py-0.5">{staffResources.length} total in your view</span>
            ...
            <section key={section.id} className={cn("space-y-3")}>
              ...
              {items.length === 0 ? (
                <div className="rounded-xl border border-dashed border-border/60 bg-card/50 p-4 text-[12px] text-muted-foreground">
                  {section.id === "videos"
                    ? "No Staffing-specific videos have been uploaded yet."
