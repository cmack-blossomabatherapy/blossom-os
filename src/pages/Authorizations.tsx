...
import { AlertTriangle, CheckCircle2, Clock, FileWarning, RefreshCw, Send, ShieldCheck } from "lucide-react";
...
export default function Authorizations() {
  ...
  const auths = useMemo<AuthRow[]>(() => clients.flatMap((client) => client.authorizations.map((auth, index) => ({ ...auth, client, authId: auth.id ?? `${client.id}-${auth.type}-${index}`, type: auth.type } as AuthRow))), [clients]);
  ...
  const filtered = useMemo(() => {
    ...
    if (view === "expiring") rows = rows.filter((auth) => auth.status === "Expired" || (daysUntil(auth.expirationDate) ?? 999) <= 90);
    return rows;
  }, [auths, query, view]);
  ...
  const metrics = {
    ...
    expiring: auths.filter((a) => (daysUntil(a.expirationDate) ?? 999) <= 90).length,
  };
  ...
  return (
    <PageShell title="Authorization" description="Revenue gate for initial auth, treatment auth, denials, and reauthorization cycles" icon={ShieldCheck}>
...
function Metric({ label, value, icon: Icon, active, onClick }: { label: string; value: number; icon: typeof ShieldCheck; active: boolean; onClick: () => void }) {
...
function AuthRevenueTable({ auths, onSelect, onSubmit, onApprove, onDeny, onMissingDocs, onReauth }: { auths: AuthRow[]; onSelect: (auth: AuthRow) => void; onSubmit: (auth: AuthRow) => void; onApprove: (auth: AuthRow) => void; onDeny: (auth: AuthRow) => void; onMissingDocs: (auth: AuthRow) => void; onReauth: (auth: AuthRow) => void }) {
  return <div className="overflow-hidden rounded-lg border border-border/60 bg-card"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b border-border bg-muted/30">{["Client", "Payor", "Type", "Status", "Days", "Expiration", "Next Action", "Alerts", "Actions"].map((header) => <th key={header} className="whitespace-nowrap px-3 py-2.5 text-left text-xs font-medium text-muted-foreground">{header}</th>)}</tr></thead><tbody>{auths.map((auth) => { const alert = getAlert(auth); const exp = daysUntil(auth.expirationDate); return <tr key={auth.authId} className="border-b border-border/40"><td className="px-3 py-3"><button onClick={() => onSelect(auth)} className="text-left font-medium text-foreground hover:text-primary">{auth.client.childName}</button><p className="text-xs text-muted-foreground">{auth.authId} · {coordinatorFor(auth.client, auth)}</p></td><td className="px-3 py-3 text-muted-foreground">{auth.payor ?? auth.client.payor}</td><td className="px-3 py-3 text-muted-foreground">{auth.type}</td><td className="px-3 py-3"><StatusBadge status={auth.status === "Not Submitted" ? "Awaiting Submission" : auth.status} variant={authVariant(auth.status)} /></td><td className="px-3 py-3 text-muted-foreground">{auth.daysInStage ?? 0}</td><td className="px-3 py-3 text-muted-foreground">{auth.expirationDate ?? "—"}{exp !== null && <span className="ml-1 text-xs">({exp}d)</span>}</td><td className="max-w-[220px] truncate px-3 py-3 text-xs text-muted-foreground">{auth.nextAction ?? "Submit Authorization"}</td><td className="px-3 py-3">{alert ? <span className={cn("inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs", alert.tone === "red" ? "bg-destructive/10 text-destructive" : "bg-warning/10 text-warning")}><AlertTriangle className="h-3 w-3" />{alert.message}</span> : <span className="text-xs text-muted-foreground">—</span>}</td><td className="px-3 py-3"><div className="flex flex-wrap gap-1.5">{auth.status === "Not Submitted" && <Button size="sm" variant="outline" onClick={() => onSubmit(auth)}>Submit</Button>}{auth.status === "Submitted" && <Button size="sm" variant="outline" onClick={() => onApprove(auth)}>Approve</Button>}<Button size="sm" variant="outline" onClick={() => onDeny(auth)}>Deny</Button>{((auth.missingDocs?.length ?? 0) > 0 || auth.requiredDocsReceived === false) && <Button size="sm" variant="outline" onClick={() => onMissingDocs(auth)}>Missing Docs</Button>}{auth.status === "Approved" && <Button size="sm" variant="outline" onClick={() => onReauth(auth)}>Reauth</Button>}</div></td></tr>; })}</tbody></table></div>{auths.length === 0 && <p className="py-10 text-center text-sm text-muted-foreground">No authorizations match this view</p>}</div>;
}