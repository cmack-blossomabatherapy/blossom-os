...
function EmptyState({ message }: { message: string }) { return <div className="rounded-lg border border-dashed border-border/70 bg-background p-6 text-center text-sm text-muted-foreground">{message}</div>; }
function EmptyOrList({ items }: { items: string[] }) { return items.length === 0 ? <EmptyState message="No live records to summarize yet." /> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <div key={item} className="rounded-lg border border-border/60 bg-background p-4 text-sm text-foreground shadow-sm">{item}</div>)}</div>; }
