...
function DashboardLoadingState({ isSyncingFirstData = false }: { isSyncingFirstData?: boolean }) {
  return <div className="space-y-6"><Section title={isSyncingFirstData ? "Syncing your first data" : "Loading live dashboard"}><div className="rounded-lg border border-primary/20 bg-primary/5 p-4"><p className="text-sm font-semibold text-foreground">{isSyncingFirstData ? "Syncing your first data from Blossom OS" : "Connecting to live Blossom OS records"}</p><p className="mt-1 max-w-3xl text-sm text-muted-foreground">{isSyncingFirstData ? "No live client records have returned yet. As soon as clients, authorizations, or service hours are added and available, this dashboard will replace these placeholders with real KPIs." : "The dashboard is initializing clients, authorizations, service hours, and leadership KPIs."}</p></div><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{["Clients", "Authorizations", "Service Hours", "Leadership KPIs"].map((label) => <div key={label} className="rounded-lg border border-border/60 bg-background p-4"><div className="mb-3 h-4 w-24 animate-pulse rounded bg-muted" /><div className="h-8 w-16 animate-pulse rounded bg-muted" /><p className="mt-3 text-xs text-muted-foreground">{isSyncingFirstData ? `Waiting for ${label.toLowerCase()}` : `Initializing ${label.toLowerCase()}…`}</p></div>)}</div></Section><Section title="Dashboard preview"><div className="space-y-3"><div className="grid gap-3 xl:grid-cols-[1.2fr_0.8fr]"><div className="h-40 animate-pulse rounded-lg bg-muted" /><div className="h-40 animate-pulse rounded-lg bg-muted" /></div><div className="h-56 animate-pulse rounded-lg bg-muted" /></div></Section></div>;
}
...
function LeadershipScorecard({ live, filteredClients, query, setQuery, sortBy, setSortBy }: { live: ReturnType<typeof buildLiveDashboard>; filteredClients: ClientRecord[]; query: string; setQuery: (value: string) => void; sortBy: keyof ClientRecord; setSortBy: (value: keyof ClientRecord) => void }) {
  const [activeDrilldown, setActiveDrilldown] = useState("active-clients");
  ...
  const activeClientRows = live.clientRecords.filter((client) => client.status === "Active");
  const pipelineClientRows = live.clientRecords.filter((client) => !["Active", "Discharged"].includes(client.status));
  const authorizedHourRows = live.clientRecords.filter((client) => client.authorizedHours > 0);
  const deliveredHourRows = live.clientRecords.filter((client) => client.deliveredHours > 0);
  const utilizationRows = live.clientRecords.filter((client) => client.authorizedHours > 0 || client.deliveredHours > 0);
  const staffingNeededRows = live.clientRecords.filter((client) => client.status === "Staffing Needed" || client.rbt === "Needed");
  ...
  const drilldowns = [
    { key: "active-clients", title: "Active Clients", description: "Clients whose current stage is Active.", rows: activeClientRows },
    { key: "pipeline-clients", title: "Clients in Pipeline", description: "Clients not yet Active or Discharged.", rows: pipelineClientRows },
    { key: "authorized-hours", title: "Authorized Hours Report", description: "Clients contributing approved authorization hours to the total.", rows: authorizedHourRows },
    { key: "delivered-hours", title: "Delivered Hours Report", description: "Clients with delivered service hours included in the metric.", rows: deliveredHourRows },
    { key: "utilization", title: "Utilization Report", description: "Clients with authorized or delivered hours used to calculate utilization.", rows: utilizationRows },
    { key: "staffing-needed", title: "Staffing Needed", description: "Clients marked Staffing Needed or missing an assigned RBT.", rows: staffingNeededRows },
    { key: "average-days-to-start", title: "Average Days to Start", description: "Clients with start dates included in the average days calculation.", rows: avgDaysToStartRows },
    { key: "revenue-estimate", title: "Revenue Estimate Report", description: "Clients with delivered hours contributing to estimated revenue.", rows: deliveredHourRows },
  ];
  const selectedDrilldown = drilldowns.find((item) => item.key === activeDrilldown) ?? drilldowns[0];

  return <>{live.clientRecords.length === 0 && <EmptyState title="No leadership data yet" message="This dashboard is connected to live Blossom OS records, but there are no clients, authorizations, or service hours to summarize yet." guidance={["Add client records with clinic, state, stage, payor, BCBA, and RBT fields.", "Add authorization records with approved hours to populate authorized hours and auth health.", "Add submitted/approved timesheets to populate delivered hours and utilization."]} />}<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><KpiCard title="Total Active Clients" value={activeClientRows.length} tone="success" active={activeDrilldown === "active-clients"} onClick={() => setActiveDrilldown("active-clients")} /><KpiCard title="Total Clients in Pipeline" value={pipelineClientRows.length} tone="info" active={activeDrilldown === "pipeline-clients"} onClick={() => setActiveDrilldown("pipeline-clients")} /><KpiCard title="Authorized Hours" value={totalAuthorized.toLocaleString()} tone="info" active={activeDrilldown === "authorized-hours"} onClick={() => setActiveDrilldown("authorized-hours")} /><KpiCard title="Delivered Hours" value={totalDelivered.toLocaleString()} tone="success" active={activeDrilldown === "delivered-hours"} onClick={() => setActiveDrilldown("delivered-hours")} /><KpiCard title="Utilization %" value={pct(utilization)} trend="Target 85%+" tone={utilization === null ? "info" : utilization >= 85 ? "success" : utilization >= 70 ? "warning" : "critical"} active={activeDrilldown === "utilization"} onClick={() => setActiveDrilldown("utilization")} /><KpiCard title="Staffing Needed" value={staffingNeededRows.length} tone={staffingNeededRows.length > 10 ? "critical" : staffingNeededRows.length > 0 ? "warning" : "success"} active={activeDrilldown === "staffing-needed"} onClick={() => setActiveDrilldown("staffing-needed")} /><KpiCard title="Average Days to Start" value={avgDaysToStart ?? "—"} tone="warning" active={activeDrilldown === "average-days-to-start"} onClick={() => setActiveDrilldown("average-days-to-start")} /><KpiCard title="Revenue Estimate" value={totalDelivered ? currency.format(totalDelivered * 80) : "—"} tone="success" active={activeDrilldown === "revenue-estimate"} onClick={() => setActiveDrilldown("revenue-estimate")} /></section><KpiDrilldownReport drilldown={selectedDrilldown} /><Section title="CEO Snapshot"><EmptyOrList items={buildSnapshot(live)} /></Section><ClinicPerformanceTable clinics={live.clinics} /><div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]"><PipelineFunnel stages={live.pipelineStages} total={Math.max(1, live.clientRecords.length)} /><ServiceSettingPerformance clients={live.clientRecords} /></div><OperationalSections live={live} /><RedFlagsAndInsights redFlags={live.redFlags} clients={live.clientRecords} /><ClientListTable clients={filteredClients} query={query} setQuery={setQuery} sortBy={sortBy} setSortBy={setSortBy} /></>;
}
...
function KpiDrilldownReport({ drilldown }: { drilldown: { title: string; description: string; rows: ClientRecord[] } }) {
  return <Section title={`${drilldown.title} Drilldown`}><p className="mb-4 text-sm text-muted-foreground">{drilldown.description}</p>{drilldown.rows.length === 0 ? <EmptyState title="No records populate this KPI yet" message="This drilldown is active, but no live records currently match the metric definition." guidance={["Add or update records for this workflow.", "When matching live data exists, the exact client list will appear here."]} /> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="text-left text-xs uppercase tracking-wide text-muted-foreground"><tr>{["Client", "Status", "Clinic", "Auth Hours", "Delivered", "Utilization", "Risk"].map((header) => <th key={header} className="px-3 py-2 font-semibold">{header}</th>)}</tr></thead><tbody className="divide-y divide-border/50">{drilldown.rows.map((client) => <tr key={client.id} className="hover:bg-muted/30"><td className="px-3 py-3 font-medium text-foreground">{client.name}</td><td className="px-3 py-3">{client.status}</td><td className="px-3 py-3">{client.clinic}</td><td className="px-3 py-3">{client.authorizedHours}</td><td className="px-3 py-3">{client.deliveredHours}</td><td className="px-3 py-3">{client.authorizedHours ? pct(Math.round((client.deliveredHours / client.authorizedHours) * 100)) : "—"}</td><td className="px-3 py-3"><RiskPill risk={client.riskLevel} /></td></tr>)}</tbody></table></div>}</Section>;
}
...
function KpiCard({ title, value, trend, tone, active, onClick }: { title: string; value: string | number; trend?: string; tone: "success" | "warning" | "critical" | "info"; active?: boolean; onClick?: () => void }) { const Icon = tone === "success" ? ArrowUp : tone === "critical" ? AlertTriangle : tone === "warning" ? ArrowDown : Users; const content = <><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium text-muted-foreground">{title}</p><Icon className={cn("h-5 w-5", tone === "success" && "text-success", tone === "warning" && "text-warning", tone === "critical" && "text-destructive", tone === "info" && "text-info")} /></div><p className="mt-3 text-3xl font-semibold tracking-tight text-foreground">{value}</p>{trend && <p className="mt-1 text-xs text-muted-foreground">{trend}</p>}{onClick && <p className="mt-2 text-xs font-medium text-primary">View drilldown</p>}</>; return onClick ? <button type="button" onClick={onClick} className={cn("rounded-xl border border-border/60 bg-card p-4 text-left shadow-sm transition-colors hover:border-primary/40 hover:bg-primary/5", active && "border-primary/50 bg-primary/5 ring-1 ring-primary/20")}>{content}</button> : <div className="rounded-xl border border-border/60 bg-card p-4 shadow-sm">{content}</div>; }
