...
export function TrainingBuilderDialog({ open, onOpenChange, onSubmit, course, courses = [] }: { open: boolean; onOpenChange: (open: boolean) => void; onSubmit: (course: TrainingCourse) => void; course?: TrainingCourse; courses?: TrainingCourse[] }) {
  ...
  const submit = (status: BuilderStatus) => {
    ...
    const next: TrainingCourse = {
      ...
      sop: { title: draft.sopTitle.trim(), content: draft.sopContent.trim(), fileName: draft.sopFileName || undefined, useAsMainContent: draft.sopAsMain },
...
function SopStep({ draft, patch, autoFormatSop }: { draft: BuilderDraft; patch: <K extends keyof BuilderDraft>(key: K, value: BuilderDraft[K]) => void; autoFormatSop: () => void }) {
  return <section className="space-y-4"><div className="rounded-xl border border-primary/30 bg-primary/10 p-3 text-sm text-primary"><FileText className="mr-2 inline h-4 w-4" />SOP is required for every training and becomes the operational source of truth.</div><Field label="SOP title"><Input value={draft.sopTitle} onChange={(e) => patch("sopTitle", e.target.value)} placeholder="VOB Handoff SOP" /></Field><Field label="SOP content editor"><Textarea value={draft.sopContent} onChange={(e) => patch("sopContent", e.target.value)} rows={12} placeholder="Paste or write the SOP. Use Auto-format to structure it into Purpose, When used, Systems required, Steps, Expected outcome, and Common mistakes." /></Field><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={autoFormatSop}><Sparkles className="mr-2 h-4 w-4" />Auto-format SOP</Button><label className="inline-flex cursor-pointer items-center rounded-md border border-input bg-background px-3 py-2 text-sm"><input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={(e) => patch("sopFileName", e.target.files?.[0]?.name ?? "")} />Upload SOP file</label>{draft.sopFileName && <StatusBadge status={draft.sopFileName} variant="default" />}</div><label className="flex items-center gap-2 text-sm text-foreground"><Checkbox checked={draft.sopAsMain} onCheckedChange={(checked) => patch("sopAsMain", Boolean(checked))} />Use SOP as main training content</label></section>;
}

function WalkthroughStep({ items, onChange }: { items: WalkthroughLink[]; onChange: (items: WalkthroughLink[]) => void }) {
  return <section className="space-y-3"><p className="text-sm text-muted-foreground">Add at least one Tango walkthrough. A preview panel appears when a URL is present.</p>{items.map((item, index) => <div key={item.id} className="rounded-xl border border-border/60 bg-background p-3"><div className="mb-3 flex items-center justify-between"><p className="font-medium text-foreground">Walkthrough {index + 1}</p><Button size="sm" variant="ghost" onClick={() => onChange(items.filter((row) => row.id !== item.id))}><Trash2 className="h-4 w-4" /></Button></div><div className="grid gap-3 md:grid-cols-[1fr_220px]"><Input value={item.url} onChange={(e) => onChange(items.map((row) => row.id === item.id ? { ...row, url: e.target.value } : row))} placeholder="https://app.tango.us/app/workflow/..." /><Input value={item.label} onChange={(e) => onChange(items.map((row) => row.id === item.id ? { ...row, label: e.target.value } : row))} placeholder="Open Walkthrough" /></div>{item.url && <div className="mt-3 rounded-lg border border-dashed border-border bg-muted/30 p-4 text-sm text-muted-foreground">Tango preview / embed placeholder · <span className="text-foreground">{item.url}</span></div>}</div>)}<Button variant="outline" onClick={() => onChange([...items, { id: uid("tango"), url: "", label: "Open Walkthrough" }])}><Plus className="mr-2 h-4 w-4" />Add Tango link</Button></section>;
}
...
function BlocksStep({ items, onChange }: { items: ContentBlock[]; onChange: (items: ContentBlock[]) => void }) {
  ...
  return <section className="space-y-3"><div className="flex flex-wrap gap-2">{types.map((type) => <Button key={type} variant="outline" size="sm" onClick={() => onChange([...items, { id: uid("block"), type, title: type, body: "" }])}><Plus className="mr-2 h-4 w-4" />{type}</Button>)}</div>{items.map((item) => <div key={item.id} className="rounded-xl border border-border/60 bg-background p-3"><div className="mb-3 flex items-center justify-between"><StatusBadge status={item.type} variant={item.type.includes("warning") ? "warning" : "info"} /><Button size="sm" variant="ghost" onClick={() => onChange(items.filter((row) => row.id !== item.id))}><Trash2 className="h-4 w-4" /></Button></div><Input value={item.title} onChange={(e) => onChange(items.map((row) => row.id === item.id ? { ...row, title: e.target.value } : row))} placeholder="Block title" /><Textarea className="mt-3" value={item.body} onChange={(e) => onChange(items.map((row) => row.id === item.id ? { ...row, body: e.target.value } : row))} rows={3} placeholder="Content, instruction, warning, or learner guidance" /></div>)}</section>;
}
