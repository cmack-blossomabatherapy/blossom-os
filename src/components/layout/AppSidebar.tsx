...
const pipelineSection: { title: string; items: NavItem[] } = {
  ...
  items: masterPipelineSections.map((section) => ({
    ...
    path: section.key === "intake" ? "/leads?view=queue" : section.key === "financial" ? "/benefits-financial" : section.key === "clientSetup" ? "/client-onboarding" : section.key === "initialAuth" ? "/authorizations" : section.key === "assessment" ? "/assessments" : `/clients?pipeline=${section.key}&view=pipeline`,
...
export function AppSidebar() {
  ...
  return (
    <aside className="w-60 h-screen sticky top-0 bg-sidebar flex flex-col border-r border-sidebar-border shrink-0">
      {/* Logo */}
      <div className="h-20 flex items-center justify-center px-3 border-b border-sidebar-border bg-primary">
        <img src={logo} alt="Blossom ABA Therapy" className="max-h-16 w-auto object-contain" />
      </div>
